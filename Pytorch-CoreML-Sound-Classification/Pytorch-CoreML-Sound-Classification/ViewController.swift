...
class ViewController: UIViewController {
 ...
 //   typealias NetworkInput = cnn10_96_nn_acc_8820Input
  //  typealias NetworkOutput = cnn10_96_nn_acc_8820Output
    var audio_model : cnn10_modified_nn_90_acc_35280? = nil
    typealias NetworkInput = cnn10_modified_nn_90_acc_35280Input
    typealias NetworkOutput = cnn10_modified_nn_90_acc_35280Output
  ...
  let spec_converter = SpectrogramConverter()
    ...
    private var N_BUFFER_TO_ANALYZE: Int=4;
  ...
  override func viewDidAppear(_ animated: Bool) {
    ...
    private func load_audio_model() -> Bool{
        do{
            ...
            self.audio_model = try cnn10_modified_nn_90_acc_35280(configuration: config)
            return true
        }
        catch{
  ...
  private func startAudioEngine() {
      ...
      inputNode.installTap(
          ...
          // 8820 * 4 -> 35280
            ...
            // Stack audio data TODO STACK FOR 4 samples = 400ms * 4 = 1.6sec
          if self.stack_buffer_i < self.N_BUFFER_TO_ANALYZE
              {
                  print("Stacking buffer \(self.stack_buffer_i) ...")
                  self.audio_data_list.append(audioData)
                  self.stack_buffer_i+=1
              }
          else { // perform inference
              // this round, we have N_BUFFER_TO_ANALYZE buffers, so we can infer alread
              print("Done stacking buffer ! Starting to analyze in continuous ..\n")
              self.audio_data_list.remove(at: 0)
              self.audio_data_list.append(audioData)
  ...
  func predict_provider(provider: MLDictionaryFeatureProvider ) {
      ...
      if let outFeatures = try? self.audio_model?.model.prediction(from: provider) {
          ...
          for key in outputs.var_239.keys{
              i+=1
              let prob = outputs.var_239[key]
